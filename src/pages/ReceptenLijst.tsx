...
export default function ReceptenLijst() {
  ...
  const selectCls = "appearance-none text-xs font-semibold text-olive-700/70 bg-white border border-olive-700/10 rounded-full pl-8 pr-7 py-2 shadow-card cursor-pointer focus:outline-none focus:ring-2 focus:ring-terracotta-600/25 hover:border-olive-700/20 transition-all"
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap items-center gap-2 mb-3">
        ...
        >
          <SlidersHorizontal size={12} /> Filters{actieveTags.length > 0 && ` (${actieveTags.length})`}
        </button>
        ...
        <div className="relative ml-auto">
          ...
          <span className="absolute right-3 top-1/2 -translate-y-1/2 text-olive-700/40 text-[10px] pointer-events-none">▼</span>
        </div>
      </div>

      {/* Gegroepeerde filterchips */}
      {toonFilters && (
        <div className="rounded-3xl bg-white border border-olive-700/8 shadow-card p-4 mb-5 space-y-3">
