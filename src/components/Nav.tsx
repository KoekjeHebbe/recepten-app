    ...
    <header className="fixed top-4 left-0 right-0 z-50 flex justify-center px-4">
      ...
      >
        <NavLink
          ...
          className="font-bold text-sm text-olive-700 px-3 py-1.5 rounded-full transition-colors hover:bg-olive-700/8 mr-1 font-serif italic tracking-tight flex-shrink-0"
        >
          TNP
        </NavLink>
        ...
        <NavLink
          to="/weekmenu"
          className={({ isActive }) =>
            `text-sm font-medium px-3 py-1.5 rounded-full transition-all btn-magnetic relative flex-shrink-0 ${isActive ? 'bg-olive-700 text-cream' : 'text-olive-700 hover:bg-olive-700/8'}`
          }
        >
          Weekmenu
          {totalItems > 0 && (
        ...
        {isIngelogd ? (
          <>
            <NavLink
              to="/recept/nieuw"
              className={({ isActive }) =>
                `text-sm font-semibold px-3 py-1.5 rounded-full transition-all btn-magnetic flex-shrink-0 ${isActive ? 'bg-terracotta-600 text-white' : 'bg-terracotta-600/10 text-terracotta-600 hover:bg-terracotta-600 hover:text-white'}`
              }
            >
              + Recept
            </NavLink>
          ...
          <NavLink
            to="/login"
            className={({ isActive }) =>
              `text-sm font-semibold px-3 py-1.5 rounded-full transition-all btn-magnetic flex-shrink-0 ${isActive ? 'bg-olive-700 text-cream' : 'text-olive-700/50 hover:text-olive-700 hover:bg-olive-700/8'}`
            }
          >
            Inloggen
          </NavLink>
